<template>
  ...
  <p v-if="language==='HTML'" id="html">Today we are learing {{language}}</p>
  <p v-else-if="language==='CSS'" id="css">Today we are learing {{language}}</p>
  <p v-else-if="language==='JS'" id="js">Today we are learing {{language}}</p>
  <p v-else>Today there's no class!</p>

  <p v-if="showBio">Mi nombre es: {{myBio.nombre}} y tengo {{myBio.edad}}</p>
  ...
  <p v-else>This is not my first time at the javascript rodeo</p>
</template>
